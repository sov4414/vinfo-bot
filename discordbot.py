...
async def on_message(message):
    ...
    if content.startswith('/v_info') or content.startswith("/v_list"):
        data = fetch_data()
        if not data:
        ...
        # === 管理者向け一覧表示 ===
        list_match = re.match(r"/v_list(?:\((\d+)\)|\s+all)?", content)
        if list_match:
            rows = data[5:]
        ...
        # No.指定（例：/v_info(1)）
        match = re.match(r'/v_info\((\d+)\)', content)
        if match:
        ...
        # テキスト検索
        query_full = content.replace('/v_info', '').strip()
        show_score = False
        ...
        if not query:
            await message.channel.send("⚠️ 検索ワードが空です。例：`/v_info ホロライブ`")
            return
